...
public class GenericDAOImpTest extends TestCase {
	...
	public void testRead() {
		try {
			Idea idea = this.daoIdea.read((long) 1);
			assertEquals(1, idea.getIdeaiId());
		} catch (DaoOperationException e) {
	...
	public void testExecuteQuery() {
		...
		try {
			...
			List<Idea> list = this.daoIdea.getByQuery(query, params, null, null);
			assertTrue((list != null && !list.isEmpty()) && list.get(0).getTitle().equals(title));
